package org.yuxinhua.weixin.util;
...
/**
 * 微信xml消息的解析器
 * 新定义的消息类型都必须在该类中增加相应的解析方法，如WxMpReceivedMessage的config_WxMpReceivedMessage方法
 * @author yxinhua
 *
 */
public class WxXStreamTransformer{
	...
	protected static final Map<Class, XStream> CLASS_2_XSTREAM_INSTANCE = configXStreamInstance();
	  ...
	  private static Map<Class, XStream> configXStreamInstance() {
	    ...
	    map.put(WxMpReceivedMessage.class, config_WxMpReceivedMessage());
	    return map;
	  }
	...
	private static XStream config_WxMpReceivedMessage() {
		XStream xstream = XStreamInitializer.getInstance();
		xstream.processAnnotations(WxMpXmlMessage.class);
		xstream.processAnnotations(WxMpReceivedMessage.class);
		xstream.processAnnotations(WxMpReceivedMessage.ScanCodeInfo.class);
		xstream.processAnnotations(WxMpReceivedMessage.SendPicsInfo.class);
		xstream.processAnnotations(WxMpReceivedMessage.SendPicsInfo.Item.class);
		xstream.processAnnotations(WxMpReceivedMessage.SendLocationInfo.class);

		xstream.aliasField("MsgID", WxMpReceivedMessage.class, "msgId");
		return xstream;
	}
